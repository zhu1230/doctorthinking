class Biz::BingliInfoController < ApplicationController
  require_role "user",:only=>[:input,:save,:add_favorite,:add_bingli_comment]
	after_filter :increment_page_views, :only => [:showOne]
	include PageViews::Controller
  def index
    #params[:keshi]=1 unless params[:keshi]
    @bingli_infos=BingliInfo.paginate :page=>params[:page],:order=>'thetime desc',:per_page => 10#,:conditions=>["keshi_id= ?",params[:keshi]]
    #@bingli_infos=BingliInfo.find(:all,:conditions=>["keshi_id= ?",params[:keshi]])
  end
def hot
	@bingli_infos=BingliInfo.paginate :page=>params[:page],:per_page => 10,:order => :page_views_counter
	render :action => "index"
end
def active
...
def week
	@bingli_infos=BingliInfo.paginate :page=>params[:page],:per_page => 10,:conditions => ['thetime > ?',1.weeks.ago],:order =>'thetime desc'
	render :action => "index"
end
def month
	@bingli_infos=BingliInfo.paginate :page=>params[:page],:per_page => 10,:conditions => ['thetime > ?',1.months.ago],:order =>'thetime desc'
	render :action => "index"
end
...
#         @bingli.bingli_info.thetime=Time.new
		...
		flash[:success]="病例发布成功！"
          redirect_to :action=>"showOne",:id=>@bingli_info.id
